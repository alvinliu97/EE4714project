...
WHERE p.id = ' . $product_id . ' AND p.id = i.productsID ORDER BY i.color ASC;';
    ...
    if ($result) {
        ...
        if ($num_rows > 0) {
            ...
            if (!isset($inventory_array[$product_color][$product_size]) || $product_qty > $inventory_array[$product_color][$product_size]) {
                $product_qty = $inventory_array[$product_color][$product_size];
                if ($product_qty < 1) {
                    $outofstock = true;
                    $add_to_cart = false;
                }
            ...
            echo ' <div class="option--quantity" id="option--quantity"' . ($outofstock ? ' style="display:none;"' : '') . '>
							<div>Quantity</div>
							<input type="number" min="1" max="' . $inventory_array[$product_color][$product_size] . '" name="quantity" class="input--text" id="product-quantity" value="' . ($product_qty > 0 ? $product_qty : 1) . '" oninput="updatePriceProduct(this)">
						</div>
						<button type="submit" class="button button--primary button--large option__button" id="button--addtocart"' . ($outofstock ? ' style="display:none;"' : '') . '>
